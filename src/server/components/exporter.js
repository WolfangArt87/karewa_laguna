...
class ExcelExporter extends Exporter {
    ...
    exportToFileExtraSheets(req, res, excelInfo){
        ...
        for(let item in excelInfo){
            ...
            columnsInfo = [
                ...
                { headers: `Consultado en Monitor Karewa Laguna`, key:'fuente', width:35}
            ];
            ...
            excelInfo[item].sheet.getRow(this.rowIndexes.INFO).getCell(3).value = `Consultado en Monitor Karewa Laguna`;
    ...
    exportToFile(req, res) {
        ...
        sheet.getRow(this.rowIndexes.INFO).getCell(3).value = `Consultado en Monitor Karewa Laguna`;
        if(this.filters && this.filters.length){
